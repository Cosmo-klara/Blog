...
*   **$S^{(l)}_{img}$**: 这代表在模型的第 $l$ 层，图像token的总体重要性分数。这个分数越高，说明该图像token在该层越重要。
*   **$\frac{1}{m} \sum^{m}_{i=1}$**: $m$ 代表文本token的数量, 对所有 $m$ 个文本token的注意力分数进行求和并取平均。
*   **$Atten^{(l)}(X_I, X^{(i)}_T)$**: 这表示在模型的第 $l$ 层，图像token $X_I$ 对第 $i$ 个文本token $X^{(i)}_T$ 的注意力分数。在视觉-语言模型中，图像token ($X_I$) 和文本token ($X_T$) 会相互作用。这里的注意力分数衡量了当模型处理第 $i$ 个文本token时，图像中的哪些部分（由图像token $X_I$ 表示）对理解该文本token最为关键。
