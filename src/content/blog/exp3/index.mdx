...
```py
ValueError: The model did not return a loss from the inputs, only the following keys: logits,rope_deltas. For reference, 
the inputs it received are input_ids,attention_mask,pixel_values_videos,video_grid_thw,video_second_per_grid,feature_attention_mask,input_features.
```