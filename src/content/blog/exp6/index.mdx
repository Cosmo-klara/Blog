...
========== Omni Batch Debug ==========
[Text]
input_ids: torch.Size([1, 8053]) # 8053 = 1500（V） + 6422（A） + 14（L）+ 117（~L）
...
[Video]
pixel_values_videos: torch.Size([6000, 1176]) # 6000 = 25 * 12 * 20; merge 后实际 token 数目 / 4 = 1500
dtype: torch.float32
video_grid_thw: tensor([[25, 12, 20]]) # 25 个视频帧( T 维度上合并了一次，初始是 50 )，每个视频帧有 12 * 20 个 patch（未合并），合并后是 6 * 10 个 patch；也就是说合并完成后一帧（对应原始两帧）对应 60 个 patch token
...
[Audio Length]
valid mel frames: [25689] # 真实音频 mel 帧数 t_real； window = 25 ms，hop = 10 ms，1 个 mel frame ≈ 0.01 s
audio seconds (approx): [256.8900146484375]  # 音频真实时长
audio tokens after encoder: [6422]  # 音频编码后的 token 数，经过两次降采样 1/4，25 fps，实际数值也就等于 t_real / 4
...
<|vision_bos|>
├─ VIDEO × 120 # 一个时间窗口内包含两帧视频帧，用这个对应的时间长度来划分时间窗口，时间窗口的大小也就是原始时间的 t_real / 50 * 2(时间压缩一次) * 2（一个窗口放两帧）
├─ AUDIO × 513 # 相同时间窗口内的对应全部音频帧，也就是 t_real / 4 * 4 / 50 = t_real / 50； 原始实现是固定 2s 内的音频 token 50 个，然后放视频的 token，视频就是原始 4 帧，时间合并一次 2 帧；
├─ VIDEO × 120
├─ AUDIO × 513 # 6422 * 4 / 50
├─ VIDEO × 120
├─ AUDIO × 513
